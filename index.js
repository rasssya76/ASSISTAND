...
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('./database/lib/myfunc')
//=================================================//
var low
try {
low = require('lowdb')
} catch (e) {
low = require('./database/lib/lowdb')}
//=================================================//
const { Low, JSONFile } = low
const mongoDB = require('./database/lib/mongoDB')
//=================================================//
...
new mongoDB(opts['db']) :
new JSONFile(`database/dbnye/database.json`)
)
...
require("./nodejs")(rama, m, chatUpdate, store)
} catch (err) {
console.log(err)}})
//=================================================//
...
const anmsu = {
document: fs.readFileSync('./database/lib/tes.xlsx'),
mimetype: feler,
...
mediaUrl: link,}}}
//=================================================//
rama.sendMessage(anu.id, anmsu, {quoted:fgclink})
...
sourceUrl: link,
mediaUrl: link,}}}
rama.sendMessage(anu.id, aalnu, {quoted:fgclink})}}
} catch (err) {
console.log(err)}})
*/
//=================================================//
...
vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await rama.getName(i + '@s.whatsapp.net')}\nFN:${await rama.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:denyp857@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://https://www.instagram.com/hikal_857/?hl=id\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`})}
//=================================================//
rama.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })}
//=================================================//
...
else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); startrama(); }
else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); startrama(); }
else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); rama.logout(); }
else if (reason === DisconnectReason.loggedOut) { console.log(`Device Logged Out, Please Scan Again And Run.`); rama.logout(); }
else if (reason === DisconnectReason.restartRequired) { console.log("Restart Required, Restarting..."); startrama(); }
else if (reason === DisconnectReason.timedOut) { console.log("Connection TimedOut, Reconnecting..."); startrama(); }
else rama.end(`Unknown DisconnectReason: ${reason}|${connection}`)}
...
// Add Other
  ...
  */
  //=================================================//
rama.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
let mime = '';
let res = await axios.head(url)
mime = res.headers['content-type']
if (mime.split("/")[1] === "gif") {
return rama.sendMessage(jid, { video: await getBuffer(url), caption: caption, gifPlayback:true, ...options}, { quoted: quoted, ...options})}
let type = mime.split("/")[0]+"Message"
if(mime === "application/pdf"){
return rama.sendMessage(jid, { document: await getBuffer(url), mimetype: 'application/pdf', caption: caption, ...options}, { quoted: quoted, ...options })}
if(mime.split("/")[0] === "image"){
return rama.sendMessage(jid, { image: await getBuffer(url), caption: caption, ...options}, { quoted: quoted, ...options})}
if(mime.split("/")[0] === "video"){
return rama.sendMessage(jid, { video: await getBuffer(url), caption: caption, mimetype: 'video/mp4', ...options}, { quoted: quoted, ...options })}
if(mime.split("/")[0] === "audio"){
return rama.sendMessage(jid, { audio: await getBuffer(url), caption: caption, mimetype: 'audio/mpeg', ...options}, { quoted: quoted, ...options })}}
//=================================================//
/** Send List Messaage
  *
  *@param {*} jid
  ...
  */
  //=================================================//
rama.sendListMsg = (jid, text = '', footer = '', title = '' , butText = '', sects = [], quoted) => {
let sections = sects
var listMes = {
text: text,
footer: footer,
...
buttonText: butText,
sections}
rama.sendMessage(jid, listMes, { quoted: quoted })}
//=================================================//
 ...
 */
 //=================================================//
rama.send5ButMsg = (jid, text = '' , footer = '', but = []) =>{
let templateButtons = but
var templateMessage = {
text: text,
footer: footer,
templateButtons: templateButtons}
rama.sendMessage(jid, templateMessage)}
//=================================================//
/** Send Button 5 Image
 *
 * @param {*} jid
 ...
 */
 //=================================================//
rama.send5ButVid = async (jid , text = '' , footer = '', vid, but = [], options = {}) =>{
let message = await prepareWAMessageMedia({ video: vid }, { upload: rama.waUploadToServer })
var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
 ...
 */
 //=================================================//
rama.send5ButGif = async (jid , text = '' , footer = '', gif, but = [], options = {}) =>{
let message = await prepareWAMessageMedia({ video: gif, gifPlayback: true }, { upload: rama.waUploadToServer })
var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
videoMessage: message.videoMessage,
...
let { writeExif } = require('./database/lib/exif')
...
ext: '.bin'}
filename = path.join(__filename, '../database/dbnye/' + new Date * 1 + '.' + type.ext)
...
data}}
//=================================================//
rama.sendFile = async(jid, PATH, fileName, quoted = {}, options = {}) => {
let types = await rama.getFile(PATH, true)
let { filename, size, ext, mime, data } = types
let type = '', mimetype = mime, pathFile = filename
if (options.asDocument) type = 'document'
if (options.asSticker || /webp/.test(mime)) {
let { writeExif } = require('./database/lib/sticker.js')
let media = { mimetype: mime, data }
...
await rama.sendMessage(jid, { [type]: { url: pathFile }, mimetype, fileName, ...options }, { quoted, ...options })
return fs.promises.unlink(pathFile)}
rama.parseMention = async(text) => {
